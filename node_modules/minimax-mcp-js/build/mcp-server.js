...
import fs from 'fs';
import { DEFAULT_BITRATE, DEFAULT_CHANNEL, DEFAULT_EMOTION, DEFAULT_FORMAT, DEFAULT_LANGUAGE_BOOST, DEFAULT_PITCH, DEFAULT_SAMPLE_RATE, DEFAULT_SPEECH_MODEL, DEFAULT_SPEED, DEFAULT_T2I_MODEL, DEFAULT_VIDEO_MODEL, DEFAULT_VOICE_ID, DEFAULT_VOLUME, ENV_CONFIG_PATH, ERROR_API_HOST_REQUIRED, ERROR_API_KEY_REQUIRED, RESOURCE_MODE_URL, } from './const/index.js';
...
export class MCPServer {
     ...
     */
    registerTextToAudioTool() {
        this.server.tool('text_to_audio', 'Convert text to audio with a given voice and save the output audio file to a given directory. If no directory is provided, the file will be saved to desktop. If no voice ID is provided, the default voice will be used.\n\nNote: This tool calls MiniMax API and may incur costs. Use only when explicitly requested by the user.', {
            ...
            subtitleEnable: z
                ...
                .optional()
                .default(false)
                .describe(`The parameter controls whether the subtitle service is enabled. The model must be 'speech-01-turbo' or 'speech-01-hd'. If this parameter is not provided, the default value is false`),
            outputFile: z
                .string()
                .optional()
    ...
    registerListVoicesTool() {
        this.server.tool('list_voices', 'List all available voices. Only supported when api_host is https://api.minimax.chat.', {
            voiceType: z
                .string()
                .optional()
                .default('all')
     ...
     */
    registerPlayAudioTool() {
        this.server.tool('play_audio', 'Play an audio file. Supports WAV and MP3 formats. Does not support video.', {
     ...
     */
    registerVoiceCloneTool() {
        this.server.tool('voice_clone', 'Clone a voice using the provided audio file. New voices will incur costs when first used.\n\nNote: This tool calls MiniMax API and may incur costs. Use only when explicitly requested by the user.', {
     ...
     */
    registerTextToImageTool() {
        this.server.tool('text_to_image', 'Generate images based on text prompts.\n\nNote: This tool calls MiniMax API and may incur costs. Use only when explicitly requested by the user.', {
    ...
    registerGenerateVideoTool() {
        this.server.tool('generate_video', 'Generate a video based on text prompts.\n\nNote: This tool calls MiniMax API and may incur costs. Use only when explicitly requested by the user.', {
            model: z
                .string()
                .optional()
     ...
     */
    registerImageToVideoTool() {
        this.server.tool('image_to_video', 'Generate a video based on an image.\n\nNote: This tool calls MiniMax API and may incur costs. Use only when explicitly requested by the user.', {
            model: z
                .string()
                .optional()
                .default('I2V-01')
     ...
     */
    registerQueryVideoGenerationTool() {
        this.server.tool('query_video_generation', 'Query the status of a video generation task.', {
            taskId: z
                .string()
     ...
     */
    registerMusicGenerationTool() {
        this.server.tool('music_generation', 'Create a music generation task using AI models. Generate music from prompt and lyrics.\n\nNote: This tool calls MiniMax API and may incur costs. Use only when explicitly requested by the user.', {
            prompt: z
                .string()
    ...
    registerVoiceDesignTool() {
        this.server.tool('voice_design', 'Generate a voice based on description prompts.\n\nNote: This tool calls MiniMax API and may incur costs. Use only when explicitly requested by the user.', {
            prompt: z
                .string()
