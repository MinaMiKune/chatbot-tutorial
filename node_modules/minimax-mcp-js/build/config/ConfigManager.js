...
import { DEFAULT_API_HOST, DEFAULT_SERVER_ENDPOINT, DEFAULT_SERVER_PORT, ENV_MINIMAX_API_HOST, ENV_MINIMAX_API_KEY, ENV_MINIMAX_MCP_BASE_PATH, ENV_RESOURCE_MODE, ENV_SERVER_ENDPOINT, ENV_SERVER_PORT, ENV_TRANSPORT_MODE, RESOURCE_MODE_URL, TRANSPORT_MODE_STDIO, } from '../const/index.js';
/**
...
export class ConfigManager {
     ...
     */
    static getConfig(requestConfig = {}, defaultConfig = {}) {
     ...
     */
    static isValidTransportMode(mode) {
        return ['stdio', 'rest', 'sse'].includes(mode);
    }
    /**
     * Apply environment variables to configuration
     */
    static applyEnvVars(config) {
        // Main configuration parameters
    ...
    static mergeConfig(target, source) {
        ...
        if (source.apiHost)
            target.apiHost = source.apiHost;
        if (source.basePath)
            target.basePath = source.basePath;
        if (source.resourceMode)
            target.resourceMode = source.resourceMode;
        // Merge server configuration
        if (source.server && target.server) {
            if (source.server.port)
                target.server.port = source.server.port;
            if (source.server.endpoint)
                target.server.endpoint = source.server.endpoint;
     ...
     */
    static extractConfigFromMetaAuth(metaAuth) {
        ...
        if (metaAuth.api_key)
            config.apiKey = metaAuth.api_key;
        if (metaAuth.api_host)
            config.apiHost = metaAuth.api_host;
        if (metaAuth.base_path)
            config.basePath = metaAuth.base_path;
        if (metaAuth.resource_mode)
            config.resourceMode = metaAuth.resource_mode;
        // Support camelCase format
        if (metaAuth.apiKey)
            config.apiKey = metaAuth.apiKey;
        if (metaAuth.apiHost)
            config.apiHost = metaAuth.apiHost;
        if (metaAuth.basePath)
            config.basePath = metaAuth.basePath;
        if (metaAuth.resourceMode)
            config.resourceMode = metaAuth.resourceMode;
